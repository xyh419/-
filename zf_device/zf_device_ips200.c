...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     显示中文字符串（汉字字符串索引）
// 参数说明     x               坐标x方向的起点 参数范围 [0, ips200_x_max-1]
// 参数说明     y               坐标y方向的起点 参数范围 [0, ips200_y_max-1]
// 参数说明     size            汉字字体大小（16表示16x16点阵）
// 参数说明     *str            中文字符串（UTF-8编码）
// 参数说明     color           颜色格式 RGB565
// 返回参数     void
// 使用示例     ips200_show_chinese_string(40, 50, 16, "真彩液晶", RGB565_BLACK);
// 备注信息     支持直接传入汉字字符串
//-------------------------------------------------------------------------------------------------------------------
void ips200_show_chinese_string(uint16 x, uint16 y, uint8 size, const char *str, const uint16 color)
{
    ...
    while(*p != '\0')
    {
        ...
        if(*p >= 0xE0 && *p <= 0xEF)
        {
            // UTF-8编码：3字节汉字 (0xE0-0xEF开头)
            font_data = chinese_font_find_by_name((const char *)p);
            p += 3;
        }
        else
        {
        ...
        if(font_data != NULL)
        {
            ips200_show_chinese(current_x, y, size, font_data, 1, color);
            current_x += size;
        }
        else
        {
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     IPS200 显示混合字符串（支持汉字、英文、数字、字符自动换行）
// 参数说明     x               坐标x方向的起点 参数范围 [0, ips200_x_max-1]
// 参数说明     y               坐标y方向的起点 参数范围 [0, ips200_y_max-1]
// 参数说明     char_size       英文字符字体大小（6表示6x8，16表示8x16）
// 参数说明     chinese_size    汉字字体大小（16表示16x16）
// 参数说明     *str            需要显示的字符串（UTF-8编码，支持汉字、英文、数字、符号）
// 参数说明     color           颜色格式 RGB565
// 返回参数     void
// 使用示例     ips200_show_mixed_string(10, 10, 16, 16, "Hello世界123中文测试", RGB565_BLACK);
// 备注信息     自动检测汉字和ASCII字符，到达屏幕边界自动换行
//-------------------------------------------------------------------------------------------------------------------
void ips200_show_mixed_string(uint16 x, uint16 y, uint8 char_size, uint8 chinese_size, const char *str, const uint16 color)
{
    ...
    uint8 char_width = (char_size == 6) ? 6 : 8;
    uint8 line_height = (char_size == 6) ? 8 : 16;
    uint8 chinese_line_height = chinese_size;
    ...
    while(*p != '\0')
    {
        ...
        if(*p >= 0xE0 && *p <= 0xEF)
        {
            ...
            if(current_x + chinese_size > ips200_x_max)
            {
                current_x = x;
                current_y += chinese_line_height;
                if(current_y + chinese_line_height > ips200_y_max)
                {
                    break;
                }
            }
            ...
            if(font_data != NULL)
            {
                ips200_show_chinese(current_x, current_y, chinese_size, font_data, 1, color);
            }
            current_x += chinese_size;
            p += 3;
        }
        else
        {
            // ASCII字符（英文、数字、符号）
            if(current_x + char_width > ips200_x_max)
            {
                current_x = x;
                current_y += line_height;
                if(current_y + line_height > ips200_y_max)
                {
                    break;
                }
            }

            ips200_show_char(current_x, current_y, *p);
