...
#define HZ_CODE(str) { (unsigned char)((str)[0]), (unsigned char)((str)[1]) }
...
typedef struct {
    uint8 name[4];                      // 汉字字符串（UTF-8编码，3字节 + 结束符）
    uint8 font_data[32];                // 16x16点阵字模数据（32字节）
} chinese_font_t;

extern const chinese_font_t chinese_font_table[];
extern const uint16 chinese_font_count;
